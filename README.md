...
product's interface from `src/components/mockups/` — plain markup and inline
SVG, no screenshots and no image files. The real products are private client
systems, so shipping screenshots of customer data was not an option; these show
the layout and the primary screen instead. They are `aria-hidden` and paired
